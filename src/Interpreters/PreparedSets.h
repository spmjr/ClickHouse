...
public:
    ...
    ConstSetPtr findOrBuild(const PreparedSetKey & key, const std::function<ConstSetPtr()> & build_set);

private:
    struct Entry
    {
        std::promise<ConstSetPtr> promise;          /// The promise is set when the set is built by the first task.
        std::shared_future<ConstSetPtr> filled_set; /// Other tasks can wait for the set to be built.
    };
