...
MarksColumnInCompressedFile::MarksColumnInCompressedFile(const PlainArray & marks, size_t base, size_t stride, size_t count)
    : num_marks(count), blocks((count + MARKS_PER_BLOCK - 1) / MARKS_PER_BLOCK)
{
    if (num_marks == 0)
    {
        return;
    }
    assert(base + stride * (num_marks - 1) < marks.size());
    ...
    for (size_t block_idx = 0; block_idx < blocks.size(); ++block_idx)
    {
        ...
        {
            const auto & mark = marks[base + (block_idx * MARKS_PER_BLOCK + i) * stride];
        ...
        block.bits_for_y = sizeof(size_t) * 8 - getLeadingZeroBits(max_y - block.min_y);
    ...
    {
        const auto & mark = marks[base + idx * stride];
        auto [block, offset] = lookUpMark(idx);
        writeBits(packed.data(), offset, mark.offset_in_compressed_file - block->min_x);
        writeBits(packed.data(), offset + block->bits_for_x, mark.offset_in_decompressed_block - block->min_y);
    }
}

MarkInCompressedFile MarksColumnInCompressedFile::get(size_t idx) const
{
    auto [block, offset] = lookUpMark(idx);
    size_t x = block->min_x + readBits(packed.data(), offset, block->bits_for_x);
    size_t y = block->min_y + readBits(packed.data(), offset + block->bits_for_x, block->bits_for_y);
    return MarkInCompressedFile{.offset_in_compressed_file = x, .offset_in_decompressed_block = y};
}

std::tuple<const MarksColumnInCompressedFile::BlockInfo *, size_t> MarksColumnInCompressedFile::lookUpMark(size_t idx) const
{
...
size_t MarksColumnInCompressedFile::approximateMemoryUsage() const
{
    return sizeof(*this) + blocks.size() * sizeof(blocks[0]) + packed.size() * sizeof(packed[0]);
}

void MarksInCompressedFile::updateApproximateMemoryUsage()
{
    approximate_memory_usage = 0;
    for (auto & c : columns)
        approximate_memory_usage += c.approximateMemoryUsage();
}
