...
TEST(Marks, Compression)
{
    ...
    auto test = [](const PODArray<MarkInCompressedFile> & plain, size_t max_bits_per_mark)
    {
        ...
        MarksColumnInCompressedFile marks(copy, 0, 1, plain.size());
        for (size_t i = 0; i < plain.size(); ++i)
            ASSERT_EQ(marks.get(i), plain[i]);

        EXPECT_LE((marks.approximateMemoryUsage() - sizeof(MarksColumnInCompressedFile)) * 8, plain.size() * max_bits_per_mark);
    };
