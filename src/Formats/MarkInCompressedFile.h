...
/**
 * An in-memory representation of the array of marks corresponding to one column of one part.
 *
 ...
 */
class MarksColumnInCompressedFile
{
public:
    using PlainArray = PODArray<MarkInCompressedFile>;

    // Takes the sequence {marks[base], marks[base + stride], marks[base + stride * 2], ...}
    // and converts it to the compressed in-memory representation.
    MarksColumnInCompressedFile(const PlainArray & marks, size_t base, size_t stride, size_t count);
...
struct MarksInCompressedFile
{
    // For compact parts, all columns of the part.
    // For wide parts, just one column.
    PODArray<MarksColumnInCompressedFile> columns;

    size_t approximate_memory_usage = UINT64_MAX;

    void updateApproximateMemoryUsage();
};
